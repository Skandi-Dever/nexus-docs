...
* **Google Drive (`drive.files`):** Utilized to programmatically create the "Nexus System" folder structure and securely store related operational files. The Software only accesses and manages files it generates; it does not read, modify, or delete unrelated private files within the Client's Google Drive.
